...
#
# $Id: featurestructure.py,v 1.8 2006/02/07 21:41:13 adastra Exp $
...
class FeatureStructure:
    ...
    def __getitem__(self, index):
        if type(index) == str:
            ...
            return self._features[self.sorted_features()[index]]
        elif len(index) == 0:
            return self
        elif len(index) == 1:
    ...
    def items(self):
        return zip(self.sorted_features(),
                   [self._features[index] for index in self.sorted_features()])
    ...
    def sorted_features(self):
        names = self._features.keys()
        names.sort() # sorting note: keys are unique strings, so we'll
    ...
    def _repr(self, reentrances, reentrance_ids):
        ...
        for fname in self.sorted_features():
            fval = self._features[fname]
