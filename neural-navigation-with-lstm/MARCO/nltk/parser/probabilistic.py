...
#
# $Id: probabilistic.py,v 1.23 2007/02/12 15:12:17 adastra Exp $
...
class ViterbiPCFGParser(AbstractParser):
    ...
    entries for constitutents that span two elements of text.  It
    ...
    def get_parse_list(self, token,guessKnownNow=0):
        ...
        # The most likely consituent table.  This table specifies the
        ...
        # sparse.
        #@#@#@# Reworked constituents table
        #@#@#@# from hash[(start,stop,rhs)] to hash[(start,rhs)][stop]
        constituents = {}
        
        #Make list of productions that may be used.
        self._productions = self._grammar._nt_grammar
        if self._guessUnknown:
            self._productions.extend(self._grammar._rindex.get('$UNKNOWN$'))
        ...
        for index in range(len(subtokens)):
            tok = subtokens[index]
            constituents.setdefault((index,tok[LEAF]), {})[index+1] = tok.copy()
            for lhs in self._grammar.keys(): constituents[index,lhs]={}
            lexiconEntry = self._grammar._rindex.get(tok[LEAF])
            if lexiconEntry: self._productions.extend(lexiconEntry)
            if (self._guessUnknown and not lexiconEntry) or guessKnownNow:
                token_text = tok[LEAF].lower()
                if self._trace and self._guessUnknown and not guessKnownNow:
                    print 'Unknown token', token_text
                lexiconEntry = self._grammar._rindex.get(token_text)
                if lexiconEntry: self._productions.extend(lexiconEntry)
                elif self._spellchecker:
                    if self._trace: print 'Using spelling correction'
                    for s in self._spellchecker.suggest(token_text):
                        if self._spellchecker.pwl.check(s):
                            lexiconEntry = self._grammar._rindex.get(s)
                            if lexiconEntry:
                                self._productions.extend(lexiconEntry)
                                subtokens[index][LEAF] = s
                                constituents.setdefault((index,s), {})[index+1] = tok.copy()
                                del constituents[(index, token_text)]
                                if self._trace: print 'Found correction', s, lexiconEntry, tok, subtokens[index]
                                break
                    if not lexiconEntry and not self._spellchecker.check(token_text):
                        token_text = self._spellchecker.suggest(token_text)[0]
                        subtokens[index][LEAF] = token_text
                        if self._trace: print 'Found correction ', token_text
                if not lexiconEntry:
                    constituents.setdefault((index,'$UNKNOWN$'),{})[index+1] = tok.copy()
            if self._trace > 1:
        ...
        for length in range(1, len(subtokens)+1):
            ...
            for start in range(len(subtokens)-length+1):
                self._add_constituents_spanning(start, start+length,
                                                constituents, subtokens)

        # Find all trees that span the entire text & have the right cat
        trees = [constituents.setdefault((0, self._grammar.start()),
                                         {}).get(len(subtokens), [])]
        ...
        if not trees or not trees[0]:
            if not self._guessUnknown:  # 
                self._guessUnknown = 1
                if self._trace: print 'Reparsing with $UNKNOWN$ words',
                trees = self.parse_n(text, 1)
                self._guessUnknown = 0
            if not trees or not trees[0] and self._guessKnown:
                if self._trace: print 'Reparsing with reinterpreting known as $UNKNOWN$',
                trees = self.parse_n(text, 1,1)
        del self._productions
        return trees

    def _add_constituents_spanning(self, start, end, constituents, subtokens):
        """
        ...
        while changed:
            ...
            # probability.
            for (production, children) in self._find_instantiations(start, end, constituents):
                ...
                conList = constituents.setdefault((start,lhs),{})
                c = conList.get(end,None)
                if self._trace > 1:
    ...
    def _find_instantiations(self, start, end, constituents):
        """
        ...
        for p in self._productions:
            production = self._grammar._productions[p]
            for childlist in self._match_rhs(production.rhs(), start, end, constituents):
                rv.append( (production, childlist) )
        return rv

    def _match_rhs(self, rhs, start, end, constituents):
        """
        ...
        # Base case
        null_rhs = not rhs
        if start >= end and null_rhs: return [[]]
        if start >= end or null_rhs: return []

        # Find everything that matches the 1st symbol of the RHS
        childlists = []
        rhs_rest = rhs[1:]
        for split,l in constituents.setdefault((start,rhs[0]),{}).items():
            ### Skip the last step of recursion for significant savings
            if split >= end and not rhs_rest:  newlist = [[l]+[]]
            elif split >= end or not rhs_rest:  newlist = []
            else: newlist = [[l]+r for r in self._match_rhs(rhs_rest, split, end, constituents)]
            childlists += newlist
        return childlists
    ...
    def __repr__(self):
        return '<ViterbiPCFGParser for %r, GuessUnknown %d>' % (self._grammar, self._guessUnknown)
