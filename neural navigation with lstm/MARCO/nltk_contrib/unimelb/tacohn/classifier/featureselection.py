...
#
# $Id: featureselection.py,v 1.2 2003/10/22 01:59:29 trevorcohn1 Exp $

# TODO - update doco re: removal of labeled text from detect
...
class SelectedFDList(AbstractFDList):
    ...
    def detect(self, text):
        # Inherit docs from AbstractFDList
        fv_list = self._sub_fd_list.detect(text)
    ...
    def ranges(self):
        old_rss = self._sub_fd_list.ranges()
        new_rss = []
        for i in range(len(old_rss)):
            r = old_rss[i]
            found = False
            for id in self._idmap.keys():
                if id in r:
                    found = True
            if found:
                new_rss.append(r)
    ...
    def describe(self, fnum):
        return self._sub_fd_lists.describe(fnum)
