...
class ProximateTokensRule(BrillRuleI):
    ...
    def apply_at(self, tokens, positions):
        ...
        for i in change:
            tokens[i][self.TAG] = self._replacement
    ...
    def applies(self, tokens, index):
        ...
        if tokens[index][self.TAG] != self._original:
            return False
...
class ProximateTokensTemplate(BrillTemplateI):
    ...
    def applicable_rules(self, tokens, index, correct_tag):
        if tokens[index][self.TAG] == correct_tag:
            return []
        ...
        return [self._rule_class(tokens[index][self.TAG], correct_tag, *conds)
